    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] p-4 sm:p-8 font-inter">
            <div className="max-w-6xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div>
                    <h1 className="text-2xl font-black text-slate-900 tracking-tight">Business Hub Manager</h1>
                    <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mt-1">Manage Strategies & Plan Control</p>
                </div>
                <div className="flex gap-3">
                    <button 
                        onClick={() => setShowSettingsModal(true)}
                        className="bg-white text-slate-600 border border-slate-200 px-6 py-3 rounded-2xl font-black text-sm flex items-center gap-2 hover:bg-slate-50 transition-all shadow-sm"
                    >
                        <Layout size={20} /> PLAN SETTINGS
                    </button>
                    <button 
                        onClick={() => { resetForm(); setShowModal(true); }}
                        className="bg-[#5D38F0] text-white px-6 py-3 rounded-2xl font-black text-sm flex items-center gap-2 shadow-lg shadow-indigo-100 hover:bg-[#4C2CD9] transition-all active:scale-95"
                    >
            ...
            <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {loading ? (
                    <div className="col-span-full flex flex-col items-center justify-center py-20 gap-4">
                        ...
                        <p className="font-black text-slate-400 uppercase tracking-widest text-xs">Loading Hub Cards...</p>
                    </div>
                ) : (
                    ideas.map((idea) => (
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex flex-wrap gap-1 mb-1">
                                        {(idea.badges || ['Trending']).map((b, i) => (
                                            <span key={i} className="text-[7px] font-black bg-emerald-50 text-emerald-500 px-1.5 py-0.5 rounded-full uppercase">{b}</span>
                                        ))}
                                    </div>
                                    <h3 className="font-black text-slate-900 truncate leading-tight">{idea.title}</h3>
                                    <p className="text-[10px] font-black text-indigo-600 uppercase mt-0.5">₹{idea.potentialEarnings}+ Monthly</p>
                                </div>
                            </div>
                            <p className="text-xs font-bold text-slate-400 line-clamp-2 leading-relaxed mb-6">{idea.desc}</p>
                            
                            <div className="flex items-center gap-2 pt-4 border-t border-slate-50">
                                <button onClick={() => openEdit(idea)} className="flex-1 bg-slate-50 text-slate-600 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-50 transition-all flex items-center justify-center gap-2"><Edit2 size={14} /> EDIT</button>
                                <button onClick={() => openEcoEditor(idea)} className="flex-1 bg-amber-50 text-amber-600 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-amber-100 transition-all flex items-center justify-center gap-2"><Layout size={14} /> ECOSYSTEM</button>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
                    <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in duration-200">
                        <div className="px-8 pt-8 pb-4 flex items-center justify-between border-b border-slate-50">
                            <div>
                                <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight">Ecosystem Content</h2>
                        ...
                        <div className="p-8 space-y-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
                            {ecoData.cards.map((card, idx) => (
                                <div key={card.id} className="space-y-3">
                                    <div className="flex items-center gap-3">
                                        <span className="w-8 h-8 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center font-black text-[10px]">0{idx+1}</span>
                                        <input
                                            type="text"
                                            value={card.title}
                                            onChange={e => {
                                            ...
                                            className="flex-1 bg-slate-50 border border-slate-100 rounded-xl px-4 py-2 text-sm font-black outline-none focus:ring-2 focus:ring-indigo-500/20"
                            ...
                            <button onClick={handleSaveEco} disabled={submitting} className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black text-[12px] uppercase tracking-[0.2em] shadow-xl flex items-center justify-center gap-3 active:scale-95 transition-all">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
                    <div className="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in duration-200 flex h-[85vh]">
                        ...
                        <div className="w-64 bg-slate-50 border-r border-slate-100 p-6 flex flex-col gap-4 overflow-y-auto">
                            <div className="mb-2">
                                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Available Plans</h3>
                            </div>
                            {settingsData.businessPlans.map((plan, idx) => (
                                ...
                                >
                                    <div className="min-w-0">
                                        <p className={`font-black text-xs truncate ${activePlanIdx === idx ? 'text-indigo-600' : 'text-slate-600'}`}>{plan.title || 'Untitled'}</p>
                            ...
                            <button 
                                onClick={addNewPlan}
                                className="w-full mt-2 bg-indigo-50 text-indigo-600 p-4 rounded-2xl font-black text-[10px] uppercase flex items-center justify-center gap-2 border border-indigo-100 border-dashed hover:bg-indigo-100 transition-all"
                            >
                        ...
                        <div className="flex-1 flex flex-col overflow-hidden bg-white">
                            <div className="px-8 pt-8 pb-4 flex items-center justify-between border-b border-slate-50 shrink-0">
                                <div>
                                    <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight">Customize Membership</h2>
                            ...
                            {settingsData.businessPlans.length > 0 ? (
                                <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        <div className="space-y-1">
                                            <label className="text-[9px] font-black text-slate-400 uppercase ml-1">Plan Title</label>
                                        ...
                                        <div className="space-y-1">
                                            <label className="text-[9px] font-black text-slate-400 uppercase ml-1">Price (₹)</label>
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between border-b border-slate-50 pb-2">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Benefit Cards</label>
                                            <button onClick={() => addBenefit(activePlanIdx)} className="text-indigo-600 font-black text-[10px] uppercase flex items-center gap-1 hover:bg-indigo-50 px-3 py-1.5 rounded-lg transition-all"><PlusCircle size={14} /> Add Benefit</button>
                                        </div>
                                                ...
                                                <div key={bIdx} className="bg-slate-50/50 border border-slate-100 p-5 rounded-[2rem] space-y-3 relative group hover:bg-white hover:shadow-lg transition-all border-dashed">
                                                    ...
                                                    <div className="grid grid-cols-2 gap-3">
                                                        <div className="col-span-2 space-y-1">
                                                            <label className="text-[8px] font-black text-slate-300 uppercase ml-1">Heading</label>
                                                        ...
                                                        <div className="space-y-1">
                                                            <label className="text-[8px] font-black text-slate-300 uppercase ml-1">Icon</label>
                                                        ...
                                                        <div className="space-y-1">
                                                            <label className="text-[8px] font-black text-slate-300 uppercase ml-1">Color</label>
                                                        ...
                                                        <div className="col-span-2 space-y-1">
                                                            <label className="text-[8px] font-black text-slate-300 uppercase ml-1">Subheading</label>
                                    ...
                                    <div className="pt-4 flex justify-between gap-4">
                                        <button onClick={() => removePlan(activePlanIdx)} className="bg-rose-50 text-rose-500 px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-rose-100 transition-all flex items-center gap-2"><Trash2 size={16} /> DELETE PLAN</button>
                                        <button onClick={handleSaveSettings} disabled={submitting} className="flex-1 bg-slate-900 text-white py-4 rounded-2xl font-black text-[12px] uppercase tracking-[0.2em] shadow-xl flex items-center justify-center gap-3 active:scale-95 transition-all">
                                ...
                                <div className="flex-1 flex flex-col items-center justify-center gap-4 text-slate-400">
                                    ...
                                    <p className="font-black text-xs uppercase tracking-widest">No plans created yet</p>
                                    <button onClick={addNewPlan} className="bg-indigo-600 text-white px-8 py-3 rounded-xl font-black text-[10px] uppercase">Create Your First Plan</button>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
                    <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in duration-200">
                        <div className="px-8 pt-8 pb-4 flex items-center justify-between border-b border-slate-50">
                            <div>
                                <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight">{editingId ? 'Update Strategy' : 'Create Strategy'}</h2>
                        ...
                        <form onSubmit={handleSaveIdea} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Card Banner</label>
                                    <div className="flex gap-4">
                                        ...
                                        <label className="flex-1 flex items-center justify-center gap-2 bg-slate-50 text-slate-600 rounded-xl font-black text-[9px] uppercase cursor-pointer hover:bg-slate-100 transition-all border border-slate-100">
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Monthly Profit</label>
                                ...
                                <div className="md:col-span-2 space-y-3">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Select Badges</label>
                                    <div className="flex flex-wrap gap-2">
                                        {['Trending', 'High Profit', 'New', 'Hot Deal', 'Low Budget'].map((badge) => (
                                            <button key={badge} type="button" onClick={() => toggleBadge(badge)} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${formData.badges.includes(badge) ? 'bg-[#5D38F0] text-white' : 'bg-slate-50 text-slate-400'}`}>{badge}</button>
                                        ))}
                                    </div>
                                </div>
                                <div className="md:col-span-2 space-y-2">
                                    <label className="text-[10px] font-black text-indigo-500 uppercase tracking-widest ml-1">🔗 Meeting Link</label>
                                    <input
                                        type="url"
